...
#include <boost/date_time/gregorian/greg_serialize.hpp> //needed if you want to serialize the date object
...
public:
	...
	boost::gregorian::date birthday; //I first tried to use std::chrono, but it was quite disapointing
	bool first_timer;
	...
	friend std::ostream& operator<< (std::ostream& stream, const sspn_member& m)
	{
		...
		if (m.first_timer)
			...
			stream << ", well known member, visited " << m.num_attended_meetups << " meetups.";
...
auto serialization_sample1() -> void
{
	...
	// save data to archive
	{
		...
		sspn_member member{ "Hong Kildong", boost::gregorian::date(1643, 8, 23), false, 3 };
...
public:
	...
	{
		stream << s.official_name << ", founded " << s.founded << ", " << s.members.size() << " members:\n";
...
auto serialization_sample2() -> void
{
	...
	{
		sspn instance{ "Seoul System Programmers Network", boost::gregorian::date(2016, 4, 6) };
		...
		instance.members.push_back(sspn_member{ "Yi Sunshin", boost::gregorian::date(1545, 4, 28), false, 5 });
		instance.members.push_back(sspn_member{ "Hong Kildong", boost::gregorian::date(1643, 8, 23), false, 3 });
