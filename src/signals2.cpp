...
auto signals2_sample1() -> void
{
	...
	sig.connect(hello());
	auto con = sig.connect(world()); //there is an optional return value (connection)
...
struct maximum
{
	...
	auto operator()(iter first, iter last) -> ty const
	{
		// in case there there is noting (no slot) connected to the signal
		if (first == last) return ty();
...
auto signals2_sample4() -> void
{
	...
	//so you can also create a combiner with a vector return value collecting all outputs
}
